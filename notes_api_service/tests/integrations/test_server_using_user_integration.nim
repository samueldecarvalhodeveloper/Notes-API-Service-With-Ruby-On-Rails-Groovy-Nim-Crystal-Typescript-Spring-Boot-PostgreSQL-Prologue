...
suite "Test Integration Of \"Server Using User\" Behavior":
  ...
  test "Test If Server Can Reach Users On Database Through Http Request":
    ...
    discard(httpClientImplementation.patch(fmt("{SERVER_HOST}{USER_ROUTE}/{createdUser.id}/"), $USER_JSON))

    let updatedUserFromDatabase = getUser(createdUser.id)

    check(USER_USERNAME == (%*updatedUserFromDatabase)[USER_USERNAME_KEY].getStr)
